...
extension UITableView {
    ...
    func dequeueReusableNibCell<C: UITableViewCell>(with cellType: C.Type, for indexPath: IndexPath) -> C where C: ClassIdentifiable & NibIdentifiable {
        guard let cell = dequeueReusableCell(withIdentifier: cellType.reuseId, for: indexPath) as? C else {
            fatalError("Couldn't dequeue a UITableViewHeaderFooterView with identifier: \(cellType.reuseId)")
        }
        return cell
    }
}
